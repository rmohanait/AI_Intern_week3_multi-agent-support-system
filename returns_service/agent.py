"""
Returns Service — exposed as an A2A (Agent-to-Agent) service.

This runs as a SEPARATE process on port 8001.
The main support_system connects to it via RemoteA2aAgent.

Start this service first:
    adk api_server returns_service --port 8001

How A2A works:
  1. This service exposes an agent card at: GET /well-known/agent.json
  2. The main system's RemoteA2aAgent reads that card to discover capabilities
  3. When a return request comes in, RemoteA2aAgent forwards it here via HTTP
  4. This agent processes it using its tools and returns the result
"""
...
def initiate_return(order_id: int, reason: str) -> dict:
    ...
    try:
        ...
        # Safety check — only update if still delivered
        result = (
            db.table("orders")
...
# This is the agent that adk api_server will serve
# ------------------------------------------------------------------
root_agent = Agent(
    name="returns_agent",
